...
\begin{description}
\item[Infer type (\texttt{Ctrl+c Ctrl+d})] This command will ask you
...
This makes sense as we want to use $\Fin\ n$ as a position
...
that we have a proof $x$ that $P$ holds and from that we need to
...
In the previous section, we showed how to prove that two natural 
numbers are equal by making use of the function
...
\emph{accumulator} to pass around the intermediate results.
\begin{AgdaAlign}
\begin{AgdaSuppressSpace}
\begin{code}[number]
...
laws for specific instances, to verify the correctness of
